...
const RecCode = require("../models/recoveryCodePat");
  ...
  if (req.header("auth-token")) {
    ...
    sequelize
            ...
            if (data.dataValues.Code === code) {
              RecCode.destroy({
                ...
                .then((data) => {
                  res
                    .status(200)
                    .json({ verifybit: 1, nextroute: "/signin/forget/v3Pat" });
                })
                .catch((error) => {
                  console.error("Failed to retrieve data : ", error);
                  res
                    .status(500)
                    .json({ verifybit: 0, nextroute: "/signin/forget/v2Pat" });
                });
            } else {
      ...
      .catch((error) => {
        ...
        res.status(500).json({ verifybit: 0, nextroute: "/signin/forget/v2Pat" });
      });
  } else {
    res.status(200).json({
      verifybit: 0,
